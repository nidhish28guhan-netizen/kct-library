...
**Repository:** backend `backend/` (Node 22 + Express 4, JSON-file store) · frontend `frontend/` (React 18.3 + Vite 5 SPA) · documentation `docs/` · metrics tool `tools/analyze.js`

## 1. Documentation set

| Document | Covers | Blueprint §§ |
|---|---|---|
| [unit-1-software-engineering-fundamentals-and-design.md](unit-1-software-engineering-fundamentals-and-design.md) | SDLC/process-model choice, requirements engineering (vision, stakeholders, FR/NFR → SRS), UML (use-case, class, sequences, copy state machine), layered architecture, data model and the JSON-persistence decision, barcode subsystem | §§1–17 |
...
| Fact | Value | Source |
|---|---|---|
| Stack | Node 22 / Express 4 / JWT + bcrypt RBAC · React 18.3 + Vite 5 SPA · JSON-file persistence (atomic tmp+rename) | `backend/package.json`, `db/store.js` |
| Roles | STUDENT · FACULTY · LIBRARIAN · ADMIN | `middleware/auth.js`, routes guards |
| Production size | 29 files · 1 498 LOC · 314 functions | `docs/metrics-report.json` |
| Complexity | avg McCabe **2.13**, max **14** (`policyRules.mergePolicies`) | same |
| API surface | 45 routes under `/api` (25 GET / 20 write), JSON error envelope | `backend/src/routes/index.js` |
| Automated tests | **121 passing, 6 suites** (5 domain/unit + 1 supertest REST integration) | `npx jest` output |
| E2E | 2 Selenium headless-Chrome journeys, 5 verified steps each, passing | `backend/tests/e2e/` |
| Coverage | 82.04 % stmts / 66.78 % branch / 75.71 % funcs / 88.13 % lines overall; domain 96.83 / repositories 97.05 / routes 85.41 (the three §26 core modules ≥ 80 %) | Istanbul, Unit III §7 |
...
npm run seed         # writes the initial dataset into backend/data/*.json
...
```bash
cd backend
npm test             # 121 tests, 6 suites (domain + supertest integration)
...
- **Design documentation** — blueprint §§ cited per section; where implementation deviates (JSON-file persistence instead of PostgreSQL), the deviation is stated as a recorded decision, not glossed over.
